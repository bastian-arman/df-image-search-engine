...
async def _directory_finder(nas_connector: SMBConnection, service_name: str, root_dir: str) -> list:
    dir_path = [root_dir]
...
async def _image_finder(nas_connector: SMBConnection, service_name: str, image_dirs: list) -> list:
    image_paths = []
...
async def normalize_embeddings(embeddings: Tensor) -> list | None:
    try:
        ...
        logging.error(f"[normalize_embeddings] Error while normalizing embedding dims: {e}")
        return None
    return normalized_embeddings


async def search_data(query: str, encoded_data: Tensor, image_paths: list, k=10) -> list|None:
    """
...
async def main():
    ...
    try:
        list_dir = await _directory_finder(nas_connector=conn, service_name=service_name, root_dir=root_dir)
        list_image = await _image_finder(nas_connector=conn, service_name=service_name, image_dirs=list_dir)
        
        processed_images=[]
        for image_paths in list_image:
            file_obj = BytesIO()
            conn.retrieveFile(service_name=service_name, path=image_paths, file_obj=file_obj)
            file_obj.seek(0)
            # print(file_obj)
        ...
        encoded_data = model.encode(processed_images, batch_size=8, convert_to_tensor=True, show_progress_bar=True)
