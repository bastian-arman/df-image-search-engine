...
async def _check_multisearch() -> bool:
    ...
    if st.session_state.get('image_description') and st.session_state.get("image_uploader"):
        logging.error("Error multi-method.")
        st.error("Error multi-method: Only one search method should be selected.")
        return True
    elif not st.session_state.get('image_description') and not st.session_state.get("image_uploader"):
        ...
        return True
    elif st.session_state['image_description'].strip() == "" and st.session_state.get("image_uploader"):
